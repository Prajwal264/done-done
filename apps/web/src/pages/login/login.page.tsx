...
const LoginPage: React.FC<ILoginPageProps> = () => {
  ...
  const login: React.MouseEventHandler<HTMLButtonElement> = useCallback(
    async (e) => {
      ...
      const loginPromise = new User().login(payload);
      ...
      const loginResponse = await loginPromise.catch(() => setLoading(false));
      setLoading(false);
      if (loginResponse) {
  ...
  return (
    <div className={styles.loginPage}>
      <form className={styles.loginForm}>
        ...
        />
        <PrimaryButton
          disabled={!!(errorMap.email || errorMap.password || !formData.email || !formData.password)}
          content="Log in"
          type="button"
          onClick={login}
          loading={loading}
        />
        <div className={styles.note}>By continuing you agree to DoneDone Terms of Service and Privacy Policy .</div>
